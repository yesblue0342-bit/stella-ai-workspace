...
# Stella Talk 백로그 — 6차 반복 (C1 대용량 동영상 resumable 업로드)

`npm test` **32/32** (upload-route 2 추가) + jsdom 통합 **6/6**:
- `lib/upload-route.js` useResumable: 3MB 이하 base64 / 초과 resumable, 21MB→resumable. 단위 2/2.
- 클라 `uploadLargeToDrive`: /api/drive-upload-url(세션)→Google 직접 PUT→/api/drive-finalize.
  3단계 호출 순서·동영상 스트리밍 URL·이미지 lh3 URL 반환 검증(jsdom 6/6).
- 서버 `api/drive-finalize.js`(공개+KST 가족폴더 보관+URL), `node --check` 통과.
- 송신 라우팅: 첨부 size>3MB → uploadLargeToDrive, 이하 → 기존 base64.

C1 인수조건(재생/다운로드)= 충족. 대용량 21MB 실업로드는 OAuth2 실환경 검증 대기.
