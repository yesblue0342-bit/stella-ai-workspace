...
# docker 권한: 현재 사용자가 docker 그룹이면 sudo 불필요(CI/비대화식 SSH에서 sudo 비번 멈춤 방지),
# 아니면 sudo docker. (GitHub Actions 자동배포는 docker 그룹 권장)
...
echo ""
echo "API 동작 확인: $DOCKER exec $NAME curl -s http://127.0.0.1:$PORT/api/health"
