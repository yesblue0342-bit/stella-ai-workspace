...
seo.이후.com 과 동일하게 NPM 리버스 프록시 + 도커 컨테이너 방식. **gpt.이후.com** 으로 서비스.
...
- Domain Names: `gpt.xn--hu5b23z.com`
...
- **Google OAuth 리디렉션 URI**: Drive 로그인을 새로 하려면 Google Cloud Console에서
  `https://gpt.이후.com/api/auth/callback` 를 승인된 리디렉션 URI에 추가.
...
## 🔧 자동배포 트러블슈팅 플레이북 (실제 겪은 순서대로)

배포 워크플로(`.github/workflows/deploy-oci.yml`)가 실패할 때 **에러 메시지로 원인을 좁힌다.** 아래는 2026-06 실제 디버깅 기록.

1. **`ssh.ParsePrivateKey: ssh: no key found`** → `OCI_SSH_KEY` 값이 개인키가 아님.
   - 공개키(`ssh-ed25519 AAAA...` / `.pub`)를 넣었거나, 모바일 복사 중 줄바꿈이 깨진 것.
   - 해결: `-----BEGIN OPENSSH PRIVATE KEY-----` ~ `-----END ... -----` **전체**(마지막 줄바꿈 포함)를 다시 붙여넣기. `.pub` 아님.

2. **`ssh: handshake failed: EOF`** (키와 무관하게 0.2초 만에 즉시) → **접속 HOST/PORT가 틀림.**
   - TCP는 붙었는데 SSH 인사 직전 닫힘 = 엉뚱한 주소/포트.
   - 흔한 원인: `OCI_SSH_HOST` 에 **Tailscale/CGNAT 사설 IP(`100.64~127.x.x`)** 를 넣음(인터넷·GitHub 러너에서 라우팅 불가), 또는 `OCI_SSH_PORT` 를 22가 아닌 값(앱 포트 등)으로 넣음.
   - 진짜 값 확인(서버에서): 공인 IP `curl -s ifconfig.me`, 포트 `sudo ss -tlnp | grep ssh`.
   - 해결: `OCI_SSH_HOST` = **공인 IP**, `OCI_SSH_PORT` = `22`(sshd LISTEN 포트). 앞뒤 공백·줄바꿈 금지.

3. **`fatal: detected dubious ownership in repository at '/opt/...'`** → 서버 repo가 **root 소유**라 `ubuntu` 가 git 못 씀(보통 `sudo git clone` 으로 받아서).
   - 해결: 워크플로 SSH 스크립트가 `sudo -n chown -R $(id -un):$(id -gn) "$APP_DIR"` + `git config --global --add safe.directory "$APP_DIR"` 로 자동 보정(현재 반영됨). OCI ubuntu 는 무비번 sudo 기본 제공.

> ✅ 정상 성공 로그 끝: `✅ 컨테이너 내부 정상 (정적 서빙 OK)` → `✅ OCI 재배포 완료` → `✅ Successfully executed commands to all hosts.`

## 마지막 1단계 — 공개 도메인 연결(NPM, 최초 1회)
컨테이너는 `npm_default` 네트워크에서 `stella-workspace:8970` 으로 뜬다. 공개 접속하려면 Nginx Proxy Manager 에서:
- Proxy Host 추가 → Domain `gpt.xn--hu5b23z.com` / Forward Host `stella-workspace` / Forward Port `8970`
- **Websockets Support 켜기**(SSE 스트리밍용) → SSL 탭에서 인증서 발급 + Force SSL.

---

## 🗄️ 메타데이터 DB OCI 동거 (Azure SQL 콜드스타트 탈출)
실 데이터는 Google Drive, **메타데이터(검색 인덱스/회원/메모리)만** Azure SQL 에 있었다.
Azure serverless 의 auto-pause 콜드스타트가 골치라 앱과 같은 OCI 서버 컨테이너로 옮긴다.

**코드는 이미 대응됨**: `lib/db.js` 가 호스트로 TLS 를 자동 판별한다.
- `*.database.windows.net`(Azure) → 암호화 O / 인증서 검증 O (기존 그대로)
- `localhost`·`stella-mssql`·사설IP(OCI 동거) → 암호화 O / **자체서명 허용**(자동)
- 명시 오버라이드: `DB_TRUST_SERVER_CERT=true|false`, `DB_ENCRYPT=true|false`

### 최초 1회 설정
```bash
ssh ubuntu@161.33.4.91
cd /opt/stella-ai-workspace
# .env 에 메타DB 값 추가(SA 비번은 8자+대소문자+숫자+기호):
...
nano .env
bash deploy/run-metadata-db-oci.sh     # stella-mssql 컨테이너 기동 + DB 생성
# 앱 재배포(자동) 후 확인:
docker exec stella-workspace curl -s http://127.0.0.1:8970/api/health
#   → target.mode == "oci-local" 이면 전환 완료. 테이블은 앱이 자동 생성(ensureSchema).
```
> ARM(OCI Ampere) 서버는 기본 이미지 `azure-sql-edge` 사용(스크립트 기본값).
> x86 서버면 `MSSQL_IMAGE=mcr.microsoft.com/mssql/server:2022-latest bash deploy/run-metadata-db-oci.sh`.
> 기존 Azure 데이터 이관이 필요하면 Azure 에서 bacpac/스크립트 export 후 stella-mssql 에 import(1회).
