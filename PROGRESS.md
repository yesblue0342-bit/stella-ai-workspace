...
### 무엇을 / 어떻게 고쳤나
- **(2) READ 키 == WRITE 키**: 이미 전부 `_v82`로 일치(검증만, 변경 불필요). `K`(index.html:272) 단일 상수로 중앙화됨.
- **(3) 옛 버전 키 자동 마이그레이션 추가(핵심)**: 사용자 브라우저에 과거 배포본이 `stella_rooms/posts/projects/users_final_vNN`
  (예: `_v81`/`_v90`)에 남긴 데이터가 있어도, 새 검색 로직이 `_v82`만 읽어 "0건"으로 보일 수 있는 시나리오를 차단.
  - `migrateLegacyStorageKeys()`(index.html, `loadData()` 진입부에서 1회 호출): localStorage 키를 스냅샷한 뒤
    같은 base의 **다른 버전 키**를 찾아 현재 `K.*` 키로 **id 기준 병합**(`mergeByIdInto`).
  - **데이터 손실 0**: id 중복은 현재 데이터 우선(스킵), 레거시 키는 삭제하지 않고 보존(되돌림 안전), 멱등(`_legacyMigrated` 가드).
- **(4) 대소문자/한글 부분일치**: `doSideSearch`가 `q.toLowerCase()` + `.includes()`로 이미 동작(한글은 대소문자 없어 부분일치 OK). 변경 불필요(검증).
- **(5) SW 캐시 버전**: `stella-v92 → v93` (사용자 브라우저가 새 코드 수신).

### 검증
- `node --check sw.js` OK. index.html 인라인 스크립트 4블록 `new Function` 구문 정상(bad 0).
- 마이그레이션 단위 테스트(mock localStorage): `_v90`/`_v81`/`_v77` 데이터 → 현재 키로 복구 + id 중복 dedupe(현재 우선)
  + 레거시 키 보존 + 멱등 = **ALL PASS**.

### 참고: 이전 진단(Case B)과의 관계
- 소스 grep은 `_v82`만 출현(아래 [2026-06-28] 섹션)했으나, 그건 **저장소 소스** 기준. 실제 **사용자 브라우저**에는
  이전 배포본의 다른 버전 키 데이터가 남을 수 있어, 마이그레이션은 그 갭을 데이터-안전하게 메운다(투기적 코드 변경 아님).
