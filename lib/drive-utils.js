...
async function extractDocx(buffer) {
  try {
    ...
    // mammoth는 ArrayBuffer 또는 Buffer 모두 지원하나 ESM에서 arrayBuffer 방식이 안정적
    const arrayBuffer = buffer.buffer.slice(buffer.byteOffset, buffer.byteOffset + buffer.byteLength);
    const r = await mammoth.extractRawText({ arrayBuffer });
    if (r && r.value && r.value.trim()) return r.value;
    // fallback: buffer 방식
    const r2 = await mammoth.extractRawText({ buffer });
    return r2.value || "";
  } catch (e) {
    // fallback: docx XML 직접 파싱 (fflate 활용)
    try {
      const zip = unzipSync(new Uint8Array(buffer));
      const bodyKey = Object.keys(zip).find(k => k === "word/document.xml");
      if (!bodyKey) return "";
      const xml = strFromU8(zip[bodyKey]);
      return stripXml(xml.replace(/<w:br[^>]*\/>/gi, "\n").replace(/<\/w:p>/gi, "\n"));
    } catch (e2) {
      return "";
    }
  }
...
async function extractPdf(buffer) {
  // 방법1: pdf-parse (ESM dynamic import)
  try {
  ...
  try {
    ...
    while ((m = re.exec(str)) !== null) {
      const block = m[0];
      const tj = block.match(/\(([^)]*)\)\s*T[jJ]/g) || [];
      tj.forEach(t => {
        const inner = t.match(/\(([^)]*)\)/);
        if (inner) texts.push(inner[1].replace(/\\n/g, "\n").replace(/\\r/g, ""));
      });
    }
