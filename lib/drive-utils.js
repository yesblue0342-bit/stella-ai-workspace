...
export async function buildDriveContextForChat(message = {}) {
  ...
  const path = detectDrivePathText(msg) || (link ? (link.fileId ? "Drive 링크(파일)" : "Drive 링크(폴더)") : "");
  ...
  if (!data || exactReadCount === 0) {
    // 링크 지정인데 못 읽었으면 키워드 폴백은 무의미("Drive 링크(파일)"로 검색됨) → 원인을 구체적으로 안내.
    if (link) {
      ...
      let reason;
      if (targetErr) {
        // getFileMeta/list 단계에서 예외 = 실제 접근 불가(권한/미존재/삭제) 또는 Drive 인증 문제
        reason = `링크의 항목에 접근하지 못했습니다: ${targetErr} (연결된 Drive 계정에 접근 권한이 있는지/삭제되지 않았는지 확인)`;
      } else if (foundCount === 0) {
        reason = `폴더는 열렸지만 안에서 읽을 파일을 찾지 못했습니다(하위 폴더만 있거나 빈 폴더). 스펙 파일이 든 하위 폴더 링크를 직접 지정해 보세요.`;
      } else {
        reason = `폴더에서 ${foundCount}개 항목을 찾았으나 텍스트를 추출하지 못했습니다.\n` + unreadReasons.join("\n");
      }
      return {
