...
export function getDriveRootId() {
  return normalizeDriveFolderId(mustEnvAny(["GOOGLE_DRIVE_FOLDER_ID", "GOOGLE_FOLDER_ID", "STELLA_DRIVE_FOLDER_ID", "DRIVE_FOLDER_ID"]));
}
...
//   → 사용자가 지정한 단일 폴더(StellaGPT/users/yesblue0342/notes)로 고정.
...
export async function getDriveRootIdSafe() {
  const v = normalizeDriveFolderId(envAny(["GOOGLE_DRIVE_FOLDER_ID", "GOOGLE_FOLDER_ID", "STELLA_DRIVE_FOLDER_ID", "DRIVE_FOLDER_ID"]));
  if (v) return v;
...
const ALL_DRIVES_LIST = { includeItemsFromAllDrives: true, supportsAllDrives: true };
...
export async function saveJsonToDrive({ folderPath = [], folderId, fileName, data = {} }) {
  const drive = getDrive();
  // folderId 지정 시 경로 생성 없이 그 폴더에 직접 저장(노트 고정 폴더용).
  const folder = folderId ? { id: folderId } : await ensurePath(folderPath);
...
export async function listJsonFromDrive({ folderPath = [], folderId, pageSize = 50 } = {}) {
  ...
  const result = await drive.files.list({ q, fields: "files(id,name,webViewLink,modifiedTime,createdTime)", orderBy: "modifiedTime desc", pageSize, ...ALL_DRIVES_LIST });
...
export async function readJsonFromDrive({ folderPath = [], folderId, fileName } = {}) {
  ...
  if (!file) return null;
  const res = await drive.files.get({ fileId: file.id, alt: "media", ...ALL_DRIVES }, { responseType: "text" });
  const text = typeof res.data === "string" ? res.data : JSON.stringify(res.data);
  return { id: file.id, name: file.name, data: JSON.parse(text) };
}

// ── 비생성(non-creating) 조회 헬퍼 ──
// ensurePath는 경로의 없는 폴더를 전부 '생성'한다. 진단/읽기 전용 코드(note-scan 등)가
// 그걸 쓰면 조회만 해도 빈 폴더가 Drive에 만들어지는 부작용이 있어, 생성 없는 변형을 제공한다.
...
async function exportGoogleFile(file) {
  ...
  try {
    const r = await drive.files.export(
      { fileId: file.id, mimeType: exportMime, ...ALL_DRIVES },
    ...
    if (exportMime !== "text/plain") {
      const r2 = await drive.files.export(
        { fileId: file.id, mimeType: "text/plain", ...ALL_DRIVES },
        { responseType: "arraybuffer" }
      );
...
async function extractDocx(buffer) {
  // 방법1: createRequire로 mammoth CJS 로드 (ESM 환경 대응)
  try {
...
export function detectDrivePathText(message = "") {
  ...
  // ★ 자연어 중첩 폴더 경로 인식: "#"나 "내 드라이브 > ..." 형식이 아니어도
  //   "구글 드라이브 폴더 내 A 폴더 하위의 B 폴더 하위에 보면 ..."처럼 자연어로 물으면
  //   경로를 인식 못해 buildDriveContextForChat이 null을 반환 → "정확한 폴더명으로 다시
  //   시도하라"는 안내만 반복하던 고질 버그. 문장에서 "<이름> 폴더" 토큰을 등장 순서대로
  //   이어붙여 경로로 재구성한다(하위/안/속/아래 같은 연결어는 무시). Drive 신호(드라이브/
  //   my·google drive/gdrive)와 "폴더"가 함께 있을 때만 시도해 일반 대화와의 오탐을 막는다.
...
// 프롬프트 문장에서 Drive 검색용 키워드를 뽑는다(링크 ID가 잘못됐을 때 실제 항목을 찾아 복구하는 용도).
// 식별자형 토큰(QM008, ZAQMR0110 등)을 최우선, 그 외 의미있는 단어(길이>=4, 한글/영숫자) 순.
...
// 성공하면 { data, target, path, pickName } 반환, 실패하면 후보 목록과 함께 null 대신 { candidates } 반환.
async function recoverDriveByKeywords(msg) {
  ...
  for (const kw of keywords) {
    ...
    const scored = hits.map(h => ({ h, score: keywords.filter(k => String(h.name || "").toLowerCase().includes(k.toLowerCase())).length, isFolder: h.mimeType === FOLDER_MIME }));
...
export async function buildDriveContextForChat(message = {}) {
  ...
  // 1차: 링크가 있으면 ID로 바로 접근, 아니면 정확 폴더 경로 해석 (기존 동작 유지)
  let target = null;
  if (link) {
    target = link;
  } else {
  ...
  if (target) {
    ...
    const isFolderTarget = !!(target && target.folderId);
  ...
  if (!data || exactReadCount === 0) {
    // 링크 지정인데 못 읽음 → 먼저 프롬프트 키워드로 실제 항목을 찾아 자동 복구를 시도한다.
    ...
    if (link) {
      ...
      const worthRecover = !!targetErr || foundCount === 0;
      if (worthRecover) {
        const rec = await recoverDriveByKeywords(msg);
        if (rec && rec.data) {
          // 복구 성공 → data/target/path를 대체하고 아래 정상 렌더 경로로 진행(return 안 함).
          data = rec.data;
          target = rec.target;
          path = `링크 ID를 찾지 못해 키워드 검색으로 대체함 → "${rec.pickName}"`;
        } else {
          const cand = (rec && rec.candidates) || [];
          const candList = [...new Set(cand.map(c => `${c.name}${c.mimeType === FOLDER_MIME ? "/(폴더)" : ""}`))].slice(0, 8);
          ...
          return {
            ...
            prompt: `\n\n[STELLA_GOOGLE_DRIVE_CONTEXT]\nDrive 링크 ID: ${link.fileId || link.folderId}\n결과: ${reason}\n[/STELLA_GOOGLE_DRIVE_CONTEXT]\n\n중요 규칙:\n- 내용을 지어내지 말고 위 결과를 사용자에게 그대로 알리세요. 추측 금지.`
          };
        }
      } else {
        // 파일은 찾았으나 추출 실패한 경우(형식/크기) — 사유를 파일별로 안내.
        const unreadReasons = (data && data.files || []).filter(f => !f.read).map(f => `${f.name}: ${f.error || "읽기 실패"}`).slice(0, 8);
        return {
          path, target: target || null, files: (data && data.files) || [],
    ...
    // 링크 케이스는 위에서 이미 복구 성공(data 설정)했거나 실패(return)했으므로, 아래 경로-기반 폴백은 링크가 아닐 때만.
    if (!link) {
  ...
  // 총량 50,000자 → 22,000자로 축소: 한글 위주 문서 50K자는 ~40K+ 토큰이 되어
  // TPM(분당 토큰) 한도가 낮은 OpenAI 조직(30K)에서 429 "Request too large"를 유발했음.
  const TOTAL_DRIVE_MAX = 22000;
  // 파일별 하한 2,500자를 두면 9개 이상 읽었을 때 총량이 22K를 넘겨(16개×2.5K≈40K)
  // 429 "Request too large"가 재발한다 → 발췌 대상 파일 수를 8개로 제한해 총량을 보장.
...
`\n\n[STELLA_GOOGLE_DRIVE_CONTEXT]
요청자가 입력한 경로: ${path}
실제로 읽은 파일 수: ${readFiles.length}${skippedRead.length ? ` (발췌 포함 ${promptFiles.length}개)` : ""}
