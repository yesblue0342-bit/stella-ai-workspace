...
export async function buildDriveContextForChat(message = {}) {
  ...
  let data;
  if (target) {
    data = await readDriveTarget({ ...target, recursive: false, maxFiles: 20 });
  } else {
    // 2차: 키워드 검색 폴백 (드라이브 전체 - 파일명 + 내용 fullText)
    ...
    try {
      ...
      hits = found.files || [];
    } catch (e) { searchErr = (e && e.message) ? e.message : String(e); hits = []; }

    if (!hits.length) {
      // 오류(주로 Drive 인증/권한)와 "일치 없음"을 구분해 정확히 안내
      const reason = searchErr
        ? `드라이브 검색 중 오류: ${searchErr} (Google Drive 연결/토큰(GOOGLE_REFRESH_TOKEN)/권한 확인 필요)`
        : `'${kw}' 키워드로 일치하는 파일/폴더가 없습니다.`;
      return {
        path, target: null, files: [],
        prompt: `\n\n[STELLA_GOOGLE_DRIVE_CONTEXT]\n검색 키워드: ${kw}\n결과: ${reason}\n[/STELLA_GOOGLE_DRIVE_CONTEXT]\n\n중요 규칙:\n- 내용을 지어내지 말고 위 결과를 사용자에게 그대로 알리세요. 추측 금지.`
      };
    }

    // 검색 결과 상위 항목을 실제로 읽기: 폴더면 폴더 내용, 파일이면 파일 내용
    const collected = [];
    let used = 0;
    for (const h of hits) {
    ...
    data = { target: { id: "search", name: `검색: ${kw}`, mimeType: "search", type: "search" }, files: collected };
  }
