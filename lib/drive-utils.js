...
async function ensureFolder(name, parentId) {
  ...
  const created = await drive.files.create({ requestBody: { name: safe, mimeType: FOLDER_MIME, parents: [parentId] }, fields: "id,name" });
  return created.data;
}
...
export async function saveJsonToDrive({ folderPath = [], fileName, data = {} }) {
  ...
  if (found.data.files?.[0]) {
    const updated = await drive.files.update({ fileId: found.data.files[0].id, media: { mimeType: JSON_MIME, body }, fields: "id,name,webViewLink,modifiedTime" });
    return { action: "updated", ...updated.data };
  }
  const created = await drive.files.create({ requestBody: { name, mimeType: JSON_MIME, parents: [folder.id] }, media: { mimeType: JSON_MIME, body }, fields: "id,name,webViewLink,modifiedTime" });
...
export async function listDriveDirectory({ folderId, scope, folderName, pageSize = 100 } = {}) {
  ...
  const result = await drive.files.list({ q, fields: "files(id,name,mimeType,webViewLink,modifiedTime,createdTime,size,parents)", orderBy: "folder,name", pageSize });
  return {
    folder: target,
...
export async function searchDrive({ query, pageSize = 20, scope, folderName, folderId } = {}) {
  const drive = getDrive();
  ...
  const result = await drive.files.list({ q, fields: "files(id,name,mimeType,webViewLink,modifiedTime,size)", orderBy: "modifiedTime desc", pageSize });
