...
function cleanupPathPart(part = "") {
  return String(part || "")
    // 끝에 붙는 "... (파일/내용/자료) (을/를) 분석/요약/정리/검색/찾아 (해줘)" 형태 제거
    //   ※ 반드시 공백 경계(\s+)에서 시작 → "영업자료"의 "자료" 같은 폴더명 일부를 잘라먹지 않음
...
export function detectDrivePathText(message = "") {
  ...
  if (hashLine) {
    ...
    //   예: "#구글드라이브폴더 3디와이/SAP 분석해줘" → "3디와이/SAP" 만 폴더 경로로 사용
    ...
    if (afterHash) {
      ...
      // 경로 구분자: '>' 또는 '/' 둘 다 허용 (사용자가 자연스럽게 '/' 사용)
...
export async function buildDriveContextForChat(message = {}) {
  ...
  console.log(`[drive-utils] buildDriveContextForChat path="${path}" target=${JSON.stringify(target)} read=${readFiles.length} unread=${unreadFiles.length}`);

  // ★ 토큰 초과 방지: 읽은 파일 전체 합산 길이 제한 (약 4만자 ≈ 1만 토큰)
  const TOTAL_BUDGET = 40000;
  let used = 0;
  const readParts = [];
  readFiles.forEach((f, i) => {
    if (used >= TOTAL_BUDGET) {
      readParts.push(`--- 읽은 파일 ${i + 1}: ${f.name} (분량 초과로 본문 생략) ---`);
      return;
    }
    let body = String(f.text || "");
    const remain = TOTAL_BUDGET - used;
    if (body.length > remain) body = body.slice(0, remain) + "\n...[전체 분량 초과로 일부 생략]";
    used += body.length;
    readParts.push(`--- 읽은 파일 ${i + 1}: ${f.name} (${f.mimeType || ""}) ---\n${body}`);
  });
  const readList = readParts.join("\n\n");
