...
// callOpenAI(messages) => Promise<{ content, tool_calls }>  — 실제 fetch는 호출부(api/codex/agent.js)가 주입.
// runTool(name, args)   => Promise<string>                  — 실제 파일/git I/O는 호출부가 주입.
// 반환: { text, steps:[{name,input,result}], done }
export async function runCodexAgentLoop({ system, prompt, callOpenAI, runTool, maxIterations = MAX_ITERATIONS }) {
  ...
  for (let i = 0; i < maxIterations; i++) {
    ...
    if (!calls.length) {
      return { text: msg.content || "", steps, done: true };
    }
