...
async function ensureSchema(pool) {
  await pool.request().query(`
    ...
    IF COL_LENGTH('cc_sessions','github_url') IS NULL
      ALTER TABLE cc_sessions ADD github_url NVARCHAR(400) NULL;
  `);
}

// PART B: 세션의 GitHub 저장 결과(폴더/커밋 URL) 기록 (graceful)
export async function setSessionGithubUrl(id, url) {
  try {
    const pool = await getPool(); await ensureSchema(pool);
    await pool.request().input("id", sql.NVarChar(120), id).input("u", sql.NVarChar(400), String(url || ""))
      .query("UPDATE cc_sessions SET github_url=@u, updated_at=SYSUTCDATETIME() WHERE id=@id");
  } catch { /* graceful */ }
}
...
export default { getMeta, setMeta, saveSession, listSessions, getSessionRow, setSessionGithubUrl };
