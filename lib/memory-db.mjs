...
export async function loadProfile(userId) {
  return withPool(async (pool) => {
    ...
    if (row && row.interests) { try { row.interests = JSON.parse(row.interests); } catch { /* ignore */ } }
    return row;
  }, null);
}
...
export async function buildMemoryContext(userId, { memLimit = 30 } = {}) {
  return withPool(async (pool) => {
    ...
    let ctx = "";
    if (p) {
      let interests = p.interests; try { interests = JSON.parse(p.interests).join(", "); } catch { /* ignore */ }
      ctx += "[USER PROFILE]\n";
