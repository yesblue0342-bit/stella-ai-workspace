...
// git 인증 헤더(토큰의 base64)가 에러 메시지에 그대로 노출되는 것을 막는 마스킹.
// execFile 에러의 기본 message/cmd는 -c http.extraheader=AUTHORIZATION: basic <base64> 를
// 그대로 포함하므로, 사용자에게 보여줄 문자열에는 반드시 이 함수를 거친 값만 써야 한다.
...
// 실패 시 원본 execFile 에러(= -c http.extraheader=... 인증 헤더를 그대로 포함)를 절대 그대로
// 던지지 않는다 — 토큰이 base64로 노출되는 사고를 막기 위해, 원래 git 서브커맨드(args, 토큰 미포함)와
// 마스킹된 stderr만으로 안전한 에러 메시지를 새로 구성해서 던진다.
// export: 테스트 전용(토큰-유출 회귀 테스트가 네트워크 없이 오프라인으로 검증하기 위함). 공개 API 아님.
export async function git(cwd, token, args, opts = {}) {
  ...
  try {
    ...
    let detail = scrubSecret((e && e.stderr) || (e && e.message) || String(e)).trim();
    // TLS CA 누락(컨테이너에 ca-certificates 미설치)은 원인이 불명확한 메시지라 진단 힌트를 덧붙인다.
    if (/server certificate verification failed|CAfile|unable to get local issuer/i.test(detail)) {
      detail += " — 서버 컨테이너에 CA 인증서(ca-certificates)가 없어 git이 GitHub TLS를 검증하지 못합니다. Dockerfile에 ca-certificates 설치가 반영됐는지 확인하세요.";
    }
    const safeErr = new Error("git " + args.join(" ") + " 실패: " + detail.slice(0, 600));
    safeErr.status = 502;
    throw safeErr;
  }
}
...
export async function destroyWorkspace(ws) {
  ...
  try { await rm(ws.base, { recursive: true, force: true }); } catch {}
}
...
export default { safeRelPath, scrubSecret, createWorkspace, destroyWorkspace, listDir, readFileRel, writeFileRel, deleteFileRel, commitAndPush };
