...
function formatWeather(place, weather, error) { if (error) return `Google Weather API 오류: ${error}`; if (!weather) return "Google Weather API 결과 없음"; return [`위치: ${place?.name || "알 수 없음"}`, `주소: ${place?.address || "없음"}`, `지도: [Google Maps](${place?.mapUrl || "#"})`, `현재 날씨: ${weather.weatherCondition?.description?.text || weather.weatherCondition?.type || "없음"}`, `기온: ${formatDegrees(weather.temperature)}`, `체감: ${formatDegrees(weather.feelsLikeTemperature)}`, `습도: ${weather.relativeHumidity ?? "없음"}%`, `강수확률: ${weather.precipitation?.probability?.percent ?? "없음"}%`, `바람: ${weather.wind?.speed?.value ?? "없음"} ${weather.wind?.speed?.unit || ""}`, `UV: ${weather.uvIndex ?? "없음"}`, `기준시각: ${weather.currentTime || "없음"}`].join("\n"); }
