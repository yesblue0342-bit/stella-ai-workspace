...
function formatPlaces(results, domestic, error) {
  ...
  const rows = results.map((item, index) => {
    ...
    const mapLink = item.link && item.link !== "#"
      ? `[${mapLabel}](${item.link})`
      : (domestic
          ? `[카카오맵](https://map.kakao.com/link/search/${encodeURIComponent(item.title)})`
          : `[구글맵](https://maps.google.com/?q=${encodeURIComponent(item.title)})`);
