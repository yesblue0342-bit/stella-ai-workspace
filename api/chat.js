...
export default async function handler(req, res) {
  ...
  try {
    ...
    const searchPayload = await prepareSearchContext({ message, search, searchProvider, searchType });
    ...
    if (isClaudeModel(model)) {
      return await callClaude(res, { model, message: enhancedMessage, originalMessage: message, history, system: enhancedSystem, searchPayload });
    }

    return await callOpenAI(res, { model, message: enhancedMessage, originalMessage: message, history, system: enhancedSystem, searchPayload });
  } catch (error) {
...
async function callOpenAI(res, { model, message, originalMessage, history, system, searchPayload }) {
  ...
  if (!apiKey) return res.status(500).json({ provider: "openai", error: "OPENAI_API_KEY not configured" });
...
async function callClaude(res, { model, message, originalMessage, history, system, searchPayload }) {
  ...
  if (!apiKey) return res.status(500).json({ provider: "claude", error: "ANTHROPIC_API_KEY not configured" });

  const payload = { model: normalizeClaudeModel(model), max_tokens: 4096, messages: buildClaudeMessages(history, message) };
  ...
  return res.status(200).json({ provider: "claude", model: data.model || payload.model, text: text || "응답 없음", usage: data.usage || null, ...searchMeta(searchPayload), originalMessage });
}
...
async function prepareSearchContext({ message, search = "auto", searchProvider = "auto", searchType = "auto" }) {
  ...
  if (smartIntent === "place" || smartIntent === "weather") {
    const smartPayload = await getSmartContextForMessage(message);

    if (smartIntent === "place") {
      const wantsReview = /후기|리뷰|블로그|평점/.test(String(message || ""));
      const needsFallback = !smartPayload.results || smartPayload.results.length < 3 || wantsReview;

      if (needsFallback) {
        const reviewResults = await safeSearch(() => runSearchProvider({
          provider: "all",
          query: `${smartPayload.query || message} 후기 리뷰`,
          type: "blog"
        }));
        const webResults = removeDuplicateLinks(reviewResults).slice(0, 5);
        if (webResults.length > 0) {
          smartPayload.results = removeDuplicateLinks([...(smartPayload.results || []), ...webResults]).slice(0, 12);
          smartPayload.context = `${smartPayload.context}\n\n보조 웹검색/후기 결과:\n${buildSearchContext(webResults)}`;
          smartPayload.provider = `${smartPayload.provider}+web_fallback`;
        }
      }
    }
  ...
  const shouldSearch = search === true || search === "true" || (String(search).toLowerCase() === "auto" && shouldUseSearch(message));
  if (!shouldSearch) return emptySearchPayload("general");
  ...
  try {
    results = await runSearchProvider({ provider: resolvedProvider, query: searchQuery, type: resolvedType });
  } catch (error) {
  ...
  return { used: true, query: searchQuery, provider: resolvedProvider, type: resolvedType, intent, results: limitedResults, error: searchError, context: buildSearchContext(limitedResults, searchError) };
}

function emptySearchPayload(intent = "general") {
  return { used: false, query: null, provider: null, type: null, intent, results: [], error: null, context: "" };
}
...
async function runSearchProvider({ provider, query, type }) {
  ...
  if (provider === "knowledge") {
    const [wikiResults, namuResults] = await Promise.all([safeSearch(() => searchWikipedia(query)), safeSearch(() => searchNamuWiki(query))]);
    return removeDuplicateLinks([...wikiResults, ...namuResults]);
  }

  if (provider === "all") {
    const [naverResults, googleResults] = await Promise.all([safeSearch(() => searchNaver(query, type)), safeSearch(() => searchGoogle(query))]);
    return removeDuplicateLinks([...naverResults, ...googleResults]);
  }
...
function shouldUseSearch(message) {
  const text = String(message || "").toLowerCase();
  const keywords = ["검색", "찾아", "찾아봐", "찾아줘", "최신", "최근", "이번 달", "오늘", "뉴스", "기사", "속보", "현재", "블로그", "후기", "리뷰", "여행", "숙소", "가격", "주가", "일정", "날씨", "공식", "문서", "api", "github", "오류", "error", "구글", "google", "네이버", "naver", "위키", "위키백과", "wikipedia", "나무위키", "namu", "프로필", "인물", "작가", "소설가", "뜻", "정의", "누구", "무엇", "정보", "#db", "#sap", "#stellagpt"];
...
function resolveSearchProvider(message, searchProvider, intent) {
  ...
  if (["naver", "google", "wiki", "wikipedia", "namu", "namuwiki", "knowledge", "all"].includes(requested)) return requested === "wikipedia" ? "wiki" : requested === "namuwiki" ? "namu" : requested;
...
function detectSearchType(message) {
  ...
  if (text.includes("블로그") || text.includes("후기") || text.includes("리뷰") || text.includes("여행") || text.includes("숙소")) return "blog";
  return "web";
}

function extractSearchQuery(message) {
  let text = String(message || "").trim();
  const removeWords = ["오늘", "최근", "최신", "현재", "지금", "이번 달", "이번달", "뉴스", "기사", "속보", "블로그", "후기", "리뷰", "구글", "google", "네이버", "naver", "위키백과", "wikipedia", "나무위키", "namu", "위키", "지식검색", "knowledge", "검색해서", "검색해줘", "검색하고", "검색", "찾아서", "찾아줘", "찾아보고", "요약해서", "요약해줘", "요약", "한 문단으로", "한문단으로", "정리해서", "정리해줘", "정리", "알려줘", "분석해줘", "설명해줘"];
  for (const word of removeWords) text = text.replaceAll(word, " ");
...
`.trim();
  ...
  if (!searchPayload.used) return [baseSystem, stellaSystem].filter(Boolean).join("\n\n");
...
3. 검색 결과가 부족하거나 광고성 글처럼 질문과 맞지 않으면 제외하거나 신뢰도를 낮게 보세요.
4. 최신 정보는 변동될 수 있으므로 영업시간, 휴무, 평점은 방문 전 지도에서 확인하라고 짧게 안내하세요.
5. 마지막에 참고 링크는 2~4개만 간단히 정리하세요.
`.trim();

  const placeInstruction = searchPayload.type === "place" ? `
장소 검색 질문입니다.
- 국내 장소 검색은 Kakao Local API 결과를 우선 신뢰하세요.
- 해외 장소 검색은 Google Places API 결과를 우선 신뢰하세요.
- 네이버/구글 웹검색은 후기 확인 또는 API 결과 부족 시 보조로만 사용하세요.
- "바로 근처", "차로 5~10분", "가볍게", "제대로 식사"처럼 사용자가 선택하기 쉽게 묶으세요.
- 가능하면 표 형태로 상황 / 추천 / 이유를 정리하세요.
`.trim() : "";

  const weatherInstruction = searchPayload.type === "weather" ? `
날씨 질문입니다.
- Google Weather API 결과를 우선 사용하세요.
- 현재 날씨, 체감온도, 강수확률, 바람, 옷차림/우산 필요 여부를 실용적으로 요약하세요.
- 출장/여행 맥락이면 이동, 복장, 우산, 외투 관점으로 조언하세요.
`.trim() : "";
...
`.trim() : "";
  ...
  return [baseSystem, stellaSystem, searchInstruction, placeInstruction, weatherInstruction, driveInstruction].filter(Boolean).join("\n\n");
}
...
아래 검색/API 결과를 참고해서 사용자가 바로 이해하고 선택할 수 있게 답변해줘.
...
function buildSearchContext(results, searchError = null) {
  ...
  return results.map((item, index) => `[${index + 1}] ${item.title}\n출처: ${item.source}\nURL: ${item.link}\n요약: ${item.snippet}\n날짜: ${item.date || "없음"}`).join("\n\n");
}

async function searchNaver(query, type = "web") {
  if (!process.env.NAVER_CLIENT_ID || !process.env.NAVER_CLIENT_SECRET) throw new Error("NAVER_CLIENT_ID or NAVER_CLIENT_SECRET not configured");
  const normalizedType = ["web", "news", "blog"].includes(type) ? type : "web";
  let path = "webkr";
  let sort = "";
  if (normalizedType === "news") { path = "news"; sort = "&sort=date"; }
  else if (normalizedType === "blog") { path = "blog"; sort = "&sort=sim"; }
  const apiUrl = `https://openapi.naver.com/v1/search/${path}.json?query=${encodeURIComponent(query)}&display=8&start=1${sort}`;
  const response = await fetch(apiUrl, { method: "GET", headers: { "X-Naver-Client-Id": process.env.NAVER_CLIENT_ID, "X-Naver-Client-Secret": process.env.NAVER_CLIENT_SECRET } });
  const data = await safeJson(response);
  if (!response.ok) throw new Error(data.errorMessage || data.message || data.raw || "Naver Search API Error");
  return (data.items || []).map((item) => ({ source: `naver_${normalizedType}`, title: removeHtml(item.title), link: item.originallink || item.link || "", snippet: removeHtml(item.description), date: item.pubDate || item.postdate || null }));
}

async function searchGoogle(query) {
  if (!process.env.GOOGLE_API_KEY || !process.env.GOOGLE_CX) throw new Error("GOOGLE_API_KEY or GOOGLE_CX not configured");
  const apiUrl = `https://www.googleapis.com/customsearch/v1?key=${encodeURIComponent(process.env.GOOGLE_API_KEY)}&cx=${encodeURIComponent(process.env.GOOGLE_CX)}&q=${encodeURIComponent(query)}&num=8`;
  const response = await fetch(apiUrl, { method: "GET" });
  const data = await safeJson(response);
  if (!response.ok) throw new Error(data.error?.message || data.raw || "Google Search API Error");
  return (data.items || []).map((item) => ({ source: "google_web", title: removeHtml(item.title), link: item.link || "", snippet: removeHtml(item.snippet), date: null }));
}

async function searchWikipedia(query) {
  const searchUrl = `https://ko.wikipedia.org/w/api.php?action=query&list=search&format=json&origin=*&srsearch=${encodeURIComponent(query)}&srlimit=5`;
  const response = await fetch(searchUrl, { method: "GET", headers: { "User-Agent": "StellaGPT/1.0" } });
  const data = await safeJson(response);
  if (!response.ok) throw new Error(data.raw || "Wikipedia API Error");
  return (data.query?.search || []).map((item) => {
    const title = item.title || "";
    return { source: "wikipedia_ko", title: removeHtml(title), link: "https://ko.wikipedia.org/wiki/" + encodeURIComponent(title.replaceAll(" ", "_")), snippet: removeHtml(item.snippet), date: item.timestamp || null };
  });
}
...
async function searchNamuWiki(query) {
  const naverResults = await safeSearch(async () => {
    ...
    const apiUrl = `https://openapi.naver.com/v1/search/webkr.json?query=${encodeURIComponent(`${query} site:namu.wiki`)}&display=5&start=1`;
    const response = await fetch(apiUrl, { method: "GET", headers: { "X-Naver-Client-Id": process.env.NAVER_CLIENT_ID, "X-Naver-Client-Secret": process.env.NAVER_CLIENT_SECRET } });
    const data = await safeJson(response);
    if (!response.ok) return [];
    return (data.items || []).filter((item) => String(item.link || "").includes("namu.wiki")).map((item) => ({ source: "namu_wiki", title: removeHtml(item.title), link: item.link || "", snippet: removeHtml(item.description), date: null }));
  });
  if (naverResults.length > 0) return naverResults;
  const googleResults = await safeSearch(() => searchGoogle(`${query} site:namu.wiki`));
  return googleResults.filter((item) => String(item.link || "").includes("namu.wiki")).map((item) => ({ ...item, source: "namu_wiki" }));
}
...
function removeDuplicateLinks(results) {
  ...
  return (results || []).filter((item) => {
    ...
    const marker = item.link || `${item.source}:${item.title}`;
