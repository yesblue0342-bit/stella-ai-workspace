...
async function handleOpenAI(res, { model, message, history, system, searchPayload }) {
  ...
  if (!apiKey) {
    return res.status(500).json({ provider: "openai", error: "OPENAI_API_KEY not configured" });
  }
...
async function handleClaude(res, { model, message, history, system, searchPayload }) {
  ...
  if (!apiKey) {
    return res.status(500).json({ provider: "claude", error: "ANTHROPIC_API_KEY not configured" });
  }
...
async function prepareSearchContext(req, { message, search, searchProvider, searchType }) {
  const shouldSearch =
    search === true ||
    search === "true" ||
  ...
  // ★ 핵심 수정: search API 실패해도 AI 응답은 정상 진행
  let results = [];
  try {
    results = await callSearchApi(req, { query, provider, type });
  } catch {
    // /api/search 없거나 오류나도 무시하고 진행
    results = [];
  }
  ...
  return {
    used: limitedResults.length > 0,   // 실제 결과 있을 때만 used=true
    query,
    provider,
    type,
...
async function callSearchApi(req, { query, provider, type }) {
  ...
  if (!response.ok) return [];           // 404 등도 빈 배열 반환
  const data = await safeJson(response);
...
function shouldUseSearch(message) {
  ...
  const keywords = [
    "검색","찾아","최신","최근","오늘","뉴스","기사","현재","가격","주가",
    "일정","날씨","맛집","여행","추천","공식","문서","api","documentation",
    "docs","github","error","오류","2026","위키","위키백과","나무위키",
    "프로필","인물","작가","소설가","뜻","정의","누구","무엇","정보",
  ];
...
function resolveSearchProvider(message, searchProvider) {
  ...
  const text = String(message || "").toLowerCase();
  if (
    text.includes("프로필") || text.includes("인물") || text.includes("작가") ||
    text.includes("소설가") || text.includes("뜻") || text.includes("정의") ||
...
function detectSearchType(message) {
  const text = String(message || "").toLowerCase();
  if (text.includes("뉴스") || text.includes("기사") || text.includes("속보") ||
      text.includes("최근") || text.includes("오늘")) return "news";
  if (text.includes("블로그") || text.includes("후기") || text.includes("맛집") ||
      text.includes("여행") || text.includes("숙소") || text.includes("리뷰")) return "blog";
  return "web";
}
...
function extractSearchQuery(message) {
  ...
  const removeWords = [
    "오늘","최근","최신","현재","지금","뉴스","기사","속보","블로그","후기","리뷰",
    "위키백과","나무위키","위키","wikipedia","namu","knowledge",
    "검색해서","검색해줘","검색하고","검색","찾아서","찾아줘","찾아보고",
    "요약해서","요약해줘","요약","정리해서","정리해줘","정리",
    "알려줘","분석해줘","설명해줘","해줘","해 봐","해봐","해",
  ];
  removeWords.forEach((w) => { text = text.replaceAll(w, " "); });
...
5. 검색 결과가 부족하면 부족하다고 말하세요.`.trim();
  return baseSystem ? `${baseSystem}\n\n${instruction}` : instruction;
}

function buildEnhancedUserMessage(message, searchPayload) {
  if (!searchPayload.used || !searchPayload.context) return String(message);
