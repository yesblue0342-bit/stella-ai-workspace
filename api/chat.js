...
async function prepareSearchContext({
  ...
  const searchQuery =
    extractSearchQuery(message, resolvedType);

  let results = [];

  if (provider === "naver") {
    results = await searchNaver(
      searchQuery,
      resolvedType
    );
  ...
  } else if (provider === "namu") {
    results = await searchNamuWiki(searchQuery);
  } else if (provider === "knowledge") {
    const wikiResults =
      await searchWikipedia(searchQuery);

    const namuResults =
      await searchNamuWiki(searchQuery);
 ...
 *
 * 예:
 * "오늘 셀트리온 뉴스 검색해서 요약해줘"
 * → "셀트리온"
 *
 * "소설가 이후 위키백과 나무위키 검색해서 알려줘"
 * → "소설가 이후"
 */
function extractSearchQuery(message, type = "web") {
  let text =
    String(message || "")
      .trim();
 ...
 *
 * 나무위키는 직접 본문을 긁지 않고,
 * 네이버 웹검색 API의 site:namu.wiki 결과만 사용
 */
