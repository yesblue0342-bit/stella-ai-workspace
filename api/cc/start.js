...
export default async function handler(req, res) {
  ...
  try {
    const { model: reqModel, prompt, title, budgetUsd } = req.body || {};
    ...
    const agentId = await MA.getOrCreateAgent(model, getMeta, setMeta);
    ...
    return res.status(200).json({ sessionId, agentId, environmentId, model, budgetUsd: budget, title: title2 });
  } catch (e) {
