...
export default async function handler(req,res){
 ...
 try{
  ...
  if(!auth)return;
  const userId=auth.uid;const roomId=safeId(b.roomId||b.id,"room");const title=clean(b.title||b.name||"새 채팅");const projectId=clean(b.projectId||"");const messages=Array.isArray(b.messages)?b.messages:[];
  const saved=await saveJsonToDrive({folderPath:["chatgpt","chats",safeId(userId,"user")],fileName:`${roomId}.json`,data:{type:"chat",userId,roomId,title,projectId,messages,updatedAt:new Date().toISOString()}});
