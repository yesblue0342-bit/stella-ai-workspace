export function detectSmartIntent(message = "") {
  ...
  if (["맛집", "식당", "밥집", "카페", "장소", "근처", "주변", "restaurant", "cafe", "nearby"].some((w) => text.includes(w))) return "place";
  return "general";
}
...
function isDomestic(message = "") {
  const text = String(message || "").toLowerCase();
  const foreign = ["보스턴", "뉴욕", "도쿄", "오사카", "삿포로", "런던", "파리", "싱가포르", "상하이", "boston", "new york", "tokyo", "osaka", "london", "paris"];
  if (foreign.some((w) => text.includes(w))) return false;
  const korea = ["한국", "국내", "서울", "인천", "송도", "연수구", "한라웨스턴파크", "부산", "대구", "대전", "광주", "제주"];
...
export async function getPlaceContext(message = "") {
  ...
  try {
    results = domestic ? await searchKakaoPlaces(query) : await searchGooglePlaces(query);
  } catch (err) {
    error = err.message || "place search error";
  }
  ...
  return {
    used: true,
    intent: domestic ? "domestic_place" : "overseas_place",
    provider: primaryProvider,
    type: "place",
    query,
    results: results.slice(0, 10),
    error,
    context: formatPlaces(results.slice(0, 10), domestic, error)
  };
}
...
export async function getWeatherContext(message = "") {
  ...
  const results = weather ? [{
    ...
    title: `${place?.name || locationQuery} 현재 날씨`,
    link: place?.mapUrl || "",
    snippet: summarizeWeather(weather),
    date: weather.currentTime || null
  }] : [];
  ...
  return {
    ...
    results,
    error,
    context: formatWeather(place, weather, error)
  };
}
...
async function searchKakaoPlaces(query) {
  ...
  return (data.documents || []).map((item) => ({
    ...
    link: item.place_url || "",
    snippet: [item.category_name, item.road_address_name || item.address_name, item.phone ? `전화 ${item.phone}` : ""].filter(Boolean).join(" · "),
...
async function searchGooglePlaces(query) {
  ...
  const response = await fetch("https://places.googleapis.com/v1/places:searchText", {
    ...
    headers: {
      ...
      "X-Goog-FieldMask": "places.displayName,places.formattedAddress,places.googleMapsUri,places.location,places.primaryTypeDisplayName,places.businessStatus"
    },
  ...
  return (data.places || []).map((item) => ({
    ...
    snippet: [item.primaryTypeDisplayName?.text, item.formattedAddress, item.businessStatus].filter(Boolean).join(" · "),
...
async function geocodeByGooglePlaces(query) {
  const list = await searchGooglePlaces(query);
  const first = list[0];
  if (!first) return null;
  return { name: first.title || query, address: first.address || "", lat: first.lat, lng: first.lng, mapUrl: first.link || "" };
}
...
async function lookupGoogleWeather(lat, lng) {
  ...
  const data = await safeJson(response);
  if (!response.ok) throw new Error(data.error?.message || data.raw || "Google Weather API Error");
  return data;
}

function formatPlaces(results, domestic, error) {
  const title = domestic ? "국내 장소 검색: Kakao Local API 우선, 부족하면 Google Places 보조" : "해외 장소 검색: Google Places API 우선, 부족하면 웹검색 보조";
  if (error && results.length === 0) return `${title}\n장소 API 오류: ${error}`;
  if (!results.length) return `${title}\n장소 API 결과 없음. 네이버/구글 웹검색 보조 필요.`;
  return [title, ...results.map((item, index) => `[${index + 1}] ${item.title}\n출처: ${item.source}\n주소: ${item.address || "없음"}\nURL: ${item.link || "없음"}\n요약: ${item.snippet || "없음"}`)].join("\n\n");
}

function formatWeather(place, weather, error) {
  if (error) return `Google Weather API 오류: ${error}`;
  if (!weather) return "Google Weather API 결과 없음";
  return [
    `위치: ${place?.name || "알 수 없음"}`,
    `주소: ${place?.address || "없음"}`,
    `현재 날씨: ${weather.weatherCondition?.description?.text || weather.weatherCondition?.type || "없음"}`,
    `기온: ${formatDegrees(weather.temperature)}`,
    `체감: ${formatDegrees(weather.feelsLikeTemperature)}`,
    `습도: ${weather.relativeHumidity ?? "없음"}%`,
    `강수확률: ${weather.precipitation?.probability?.percent ?? "없음"}%`,
    `바람: ${weather.wind?.speed?.value ?? "없음"} ${weather.wind?.speed?.unit || ""}`,
    `UV: ${weather.uvIndex ?? "없음"}`,
    `기준시각: ${weather.currentTime || "없음"}`
  ].join("\n");
}

function summarizeWeather(weather) {
  return `${weather.weatherCondition?.description?.text || "날씨 정보"}, 기온 ${formatDegrees(weather.temperature)}, 체감 ${formatDegrees(weather.feelsLikeTemperature)}, 강수확률 ${weather.precipitation?.probability?.percent ?? "없음"}%`;
}

function formatDegrees(value) {
  if (!value || typeof value.degrees !== "number") return "없음";
  return `${Math.round(value.degrees * 10) / 10}°${value.unit === "FAHRENHEIT" ? "F" : "C"}`;
}

function dedupe(results) {
  const seen = new Set();
  return (results || []).filter((item) => {
    const marker = item.link || `${item.source}:${item.title}:${item.address}`;
    if (!item?.title || seen.has(marker)) return false;
    seen.add(marker);
    return true;
  });
}
