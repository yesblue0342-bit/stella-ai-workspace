...
s = s.replace(/<!-- stella-fixed-[^>]*-->/, '<!-- stella-fixed-2026-06-13-clean-download-table-ocr-sidebar-v2 -->');
...
s = s.replace("window.addEventListener('DOMContentLoaded',()=>{initEvents();initAuth();syncSidebarLayout()});", "window.addEventListener('DOMContentLoaded',()=>{if(typeof ensureModelOptions==='function')ensureModelOptions();initEvents();initAuth();syncSidebarLayout()});");
...
if (s !== before) {
  ...
  console.log('patched index.html clean downloads/table/ocr/sidebar v2');
} else {
