...
## 2026-06-22 (iter 33) · 저장 확장자 = 첨부/실제언어 기준(하드코딩 abap 제거) · pass 141/141
- 요청: "첨부 확장자는 첨부한 것으로" → 강제 .abap 폐지. resolveExt: ①첨부/명시 확장자(이미지 제외) ②코드펜스 언어(LANG_EXT 30+종) ③ABAP 키워드 ④txt.
- codex: 첨부파일명 확장자(attExt)를 ext로 전송. cc: 서버가 코드펜스로 판별(에이전트 출력). abap.html은 도메인상 ext:'abap' 유지.
- github-store +6(15/15) + 전체 141/141 · node --check OK · codex/cc 인라인·모듈 파싱 OK · 시크릿 0 · sw v76→v77.
요약 3줄:
