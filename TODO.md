...
## 전수 검사 (iter 12, autopilot)
- [x] P1 구문·정적 검사 전수 — api/lib node --check 0 fail, HTML 19 인라인 파싱 bad=0.
- [x] P2 시크릿 스캔 — 하드코딩 시크릿 0, 전부 process.env 참조.
- [x] P3 공용 인프라 — Drive 자동리프레시·Azure 타임아웃/재시도·SW 단일캐시 정상(env 외 수정 없음).
